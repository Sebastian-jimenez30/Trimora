...
function normalizeYear(value?: string) {
  ...
  return Number.isInteger(parsed) && parsed >= 2000 && parsed <= currentYear + 1 ? parsed : currentYear;
}
...
function createRange(year: number, startMonth: number, months: number) {
  ...
  const start = fromZonedTime(`${normalizedStartYear}-${String(normalizedStartMonth + 1).padStart(2, "0")}-01T00:00:00`, TIMEZONE);
  const end = fromZonedTime(`${endYear}-${String(endMonth + 1).padStart(2, "0")}-01T00:00:00`, TIMEZONE);
  return { start, end };
}

export function resolveAnalyticsPeriod(filters: AnalyticsFilters): AnalyticsPeriod & { start: Date; end: Date; previousStart: Date; previousEnd: Date } {
  ...
  const defaultSegment = type === "month" ? Number(new Intl.DateTimeFormat("en-US", { timeZone: TIMEZONE, month: "numeric" }).format(new Date())) : 1;
  const segment = Number.isInteger(requestedSegment) && requestedSegment >= 1 && requestedSegment <= maxSegment ? requestedSegment : defaultSegment;
...
function toRanking(rows: { id: string; label: string; value: string | number; secondary: string | number }[]): RankingItem[] {
  return rows.map((row) => ({ id: row.id, label: row.label, value: Number(row.value), secondary: Number(row.secondary) }));
}
...
async function getMovementTrace(organizationId: string, start: Date, end: Date, cursorValue?: string) {
  ...
  const [itemRows, paymentRows, logRows] = await Promise.all([
    ...
    db.select().from(transactionPayments).where(inArray(transactionPayments.transactionId, transactionIds)).orderBy(transactionPayments.createdAt),
    db.select({ entityId: auditLogs.entityId, details: auditLogs.details }).from(auditLogs).where(and(
  ...
  const serviceIds = [...new Set(itemRows.filter((item) => item.itemType === "SERVICE").map((item) => item.itemId))];
  const productIds = [...new Set(itemRows.filter((item) => item.itemType === "PRODUCT").map((item) => item.itemId))];
  const [serviceRows, productRows] = await Promise.all([
    serviceIds.length ? db.select({ id: services.id, name: services.name }).from(services).where(and(eq(services.organizationId, organizationId), inArray(services.id, serviceIds))) : [],
    productIds.length ? db.select({ id: products.id, name: products.name }).from(products).where(and(eq(products.organizationId, organizationId), inArray(products.id, productIds))) : [],
  ]);
...
export async function getAnalyticsData(organizationId: string, filters: AnalyticsFilters): Promise<AnalyticsData> {
  ...
  const [outstandingRows, serviceRows, productRows, clientRows, expenseRows, demandRows, yearRows, trace] = await Promise.all([
    ...
    db.select({
      ...
      .orderBy(desc(sql`sum(${transactionItems.quantity})`)).limit(6),
    db.select({
      id: transactionItems.itemId,
    ...
    db.select({
      ...
      .orderBy(desc(sql`sum(${transactions.totalAmount})`)).limit(6),
    db.select({
    ...
    db.select({ year: sql<number>`extract(year from timezone('America/Bogota', ${transactions.createdAt}))::int` })
      .from(transactions).where(eq(transactions.organizationId, organizationId))
  ...
  return {
    ...
    availableYears: [...new Set([resolved.year, ...yearRows.map((row) => Number(row.year))])].sort((a, b) => b - a),
    metrics: {
      income,
      expenses,
      net: income - expenses,
    ...
    demand: demandRows.map((row): DemandPoint => ({ day: Number(row.day), hour: Number(row.hour), value: Number(row.value) })),
