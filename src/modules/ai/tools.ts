...
export function getAiTools(context: {
  ...
  const tools = {
    ...
    agendar_cita: tool({
      ...
      inputSchema: z.object({
        ...
        date: z
          .string()
          .describe(
            "La fecha y hora en formato ISO 8601 con zona horaria (SIEMPRE usar -05:00 al final, ej. 2026-07-17T09:00:00-05:00)",
          ),
        customerNameOverride: z
          .string()
          .optional()
          .describe(
            "Si la persona está agendando para un amigo, el nombre del amigo. Si es para él mismo, dejar vacío.",
          ),
      }),
      execute: async (args) => {
    ...
    listar_servicios: tool({
      description:
        "Devuelve la lista de servicios disponibles en la barbería con sus precios y duración. Útil cuando el cliente pregunta qué ofrecen o cuánto cuesta.",
      inputSchema: z.object({}),
      execute: async () => {
    ...
    consultar_agenda_hoy: tool({
      ...
      execute: async () => {
        ...
        return todayAppointments
          .map((a) => {
            const time = new Date(a.startTime.getTime() - 5 * 60 * 60 * 1000).toLocaleTimeString(
              [],
    ...
    consultar_finanzas_hoy: tool({
      description:
        "ADMIN: Consulta el resumen financiero y el desglose completo de ingresos y gastos de hoy (o de cualquier otro día). Muestra cada transacción con su concepto, monto, método de pago y hora. Úsalo cuando pidan: resumen del día, cuánto se hizo hoy, desglose de caja, ingresos de hoy, etc.",
      inputSchema: z.object({
        fecha: z
          .string()
          .optional()
          .describe(
            'Fecha a consultar en formato YYYY-MM-DD (ej. "2026-07-17"). Si no se especifica, se usa hoy.',
          ),
      }),
      execute: async (args) => {
        ...
        txs.forEach((tx, i) => {
          const hora = new Date(tx.createdAt.getTime() - 5 * 60 * 60 * 1000).toLocaleTimeString(
            "es-CO",
            { hour: "2-digit", minute: "2-digit" },
          );
          ...
          if (tx.type === "INCOME") {
            totalIngresos += monto;
            ingresos.push(
              `  ${i + 1}. ${hora}${metodo} $${monto.toLocaleString("es-CO")} — ${concepto}`,
            );
          } else {
            totalGastos += monto;
            gastos.push(
    ...
    registrar_transaccion_caja: tool({
      ...
      inputSchema: z.object({
        type: z
          .enum(["INCOME", "EXPENSE"])
          .describe(
            "INCOME para ingresos (cobros, ventas), EXPENSE para gastos (compras, sueldos).",
          ),
      ...
      execute: async (args) => {
        ...
        try {
          ...
          return `Error registrando transacción: ${getErrorMessage(error)}`;
        }
      },
    }),
    ...
    registrar_venta_producto: tool({
      description:
        'ADMIN: Registra la venta de uno o más productos consultando automáticamente el precio en la base de datos y calculando el total. Úsalo cuando digan "registra la venta de X unidades de [producto]", "vendí 2 lociones", "registra venta de X [producto]", etc. NO necesitas saber el precio de antemano, lo busca solo.',
      inputSchema: z.object({
        nombreProducto: z
          .string()
          .describe(
            'Nombre del producto vendido (búsqueda parcial, ej. "loción", "pomade", "shampoo").',
          ),
        cantidad: z.number().int().positive().describe("Cantidad de unidades vendidas."),
        paymentMethod: z
          .string()
          .optional()
          .describe("Método de pago: CASH, CARD o TRANSFER. Por defecto CASH."),
      ...
      execute: async (args) => {
        ...
        try {
          ...
          await db.transaction(async (databaseTransaction) => {
            const [lockedProduct] = await databaseTransaction
              .select()
              .from(products)
              .where(
                and(
            ...
            if (!lockedProduct)
              throw new Error(`El producto "${producto.name}" ya no está disponible.`);

            const previousStock = Number(lockedProduct.currentStock);
            if (previousStock < args.cantidad) {
              throw new Error(
                `Stock insuficiente para ${lockedProduct.name}. Disponible: ${previousStock}.`,
              );
            }
          ...
          return `Error registrando venta: ${getErrorMessage(error)}`;
        }
      },
    }),
    ...
    crear_producto: tool({
      ...
      inputSchema: z.object({
        ...
        category: z
          .enum(["VENTA", "CONSUMO"])
          .describe(
            "VENTA si es para vender a clientes, CONSUMO si es de uso interno de la barbería.",
          ),
        price: z
          .number()
          .describe("Precio de venta (si es para venta). Poner 0 si es solo de consumo."),
      ...
      execute: async (args) => {
        ...
        try {
          ...
          return `Error creando producto: ${getErrorMessage(error)}`;
        }
      },
    }),
    ...
    crear_servicio: tool({
      ...
      execute: async (args) => {
        ...
        try {
          ...
          return `Error creando servicio: ${getErrorMessage(error)}`;
        }
      },
    }),
    ...
    consultar_productos: tool({
      description:
        "ADMIN: Consulta productos del inventario con stock, precios y categoría. Usar cuando pregunten por precios, existencias, o qué productos hay disponibles. También sirve para buscar el precio de un producto específico.",
      inputSchema: z.object({
        nombre: z
          .string()
          .optional()
          .describe(
            'Buscar un producto por nombre (búsqueda parcial, ej. "loción"). Dejar vacío para ver todos.',
          ),
        categoria: z
          .enum(["VENTA", "CONSUMO", "TODOS"])
          .optional()
          .describe(
            "Filtrar por categoría: VENTA (para clientes), CONSUMO (uso interno), o TODOS.",
          ),
      }),
      execute: async (args) => {
        ...
        const allProducts = await db.query.products.findMany({
          where: args.nombre
            ? and(
                eq(products.organizationId, context.organizationId),
                ilike(products.name, `%${args.nombre}%`),
              )
            : eq(products.organizationId, context.organizationId),
        });
        ...
        if (filtered.length === 0)
          return args.nombre
            ? `No se encontró ningún producto llamado "${args.nombre}".`
        ...
        const allClients = await db.query.clients.findMany({
          where: args.nombre
            ? and(
                ...
                ilike(clients.firstName, `%${args.nombre}%`),
              )
            : eq(clients.organizationId, context.organizationId),
        });
    ...
    consultar_transacciones: tool({
      description:
        "ADMIN: Consulta el historial de transacciones (ventas/gastos). Puede filtrar por rango de días.",
      inputSchema: z.object({
        dias: z
          .number()
          .optional()
          .describe(
            "Número de días hacia atrás a consultar (ej. 7 para última semana, 30 para el mes). Por defecto 7.",
          ),
      }),
      execute: async (args) => {
    ...
    consultar_citas: tool({
      description:
        "ADMIN: Consulta citas agendadas. Puede ver las de hoy, mañana, o los próximos días.",
      inputSchema: z.object({
        dias: z
          .number()
          .optional()
          .describe(
            "Número de días hacia adelante a consultar (1 = solo hoy, 7 = próxima semana). Por defecto 1.",
          ),
      }),
      execute: async (args) => {
