...
const importRequestSchema = z.object({
  entityType: z.enum(["products", "services"]),
...
const productSchema = z.object({
  items: z.array(
    z.object({
      ...
      category: z
        .enum(["VENTA", "CONSUMO"])
        .describe("VENTA si se vende a clientes, CONSUMO si es de uso interno"),
...
const serviceSchema = z.object({
  items: z.array(
    z.object({
      ...
      durationMinutes: z
        .number()
        .describe("Duración aproximada en minutos (ej. 30, 45, 60). Por defecto 30"),
...
export async function POST(req: Request) {
  try {
    await requireActor({ roles: ["ADMIN"] });
    const parsedBody = importRequestSchema.safeParse(await req.json());
    ...
    const { entityType, data, isImage } = parsedBody.data;

    // Configure the AI model (gpt-4o-mini is multimodal and extremely cheap)
    const model = openai("gpt-4o-mini");

    let systemPrompt =
      "Se te proporcionará información importada. Tu trabajo es limpiar, estandarizar y extraer los datos requeridos. Ignora datos inválidos.";
    ...
    if (!items || items.length === 0) {
      return NextResponse.json(
        { success: false, error: "No se encontraron elementos válidos para importar." },
        { status: 400 },
      );
    }
