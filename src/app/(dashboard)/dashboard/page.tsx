...
import { appointments, clients, services, transactions, products } from "@/core/database/schema";
...
export default async function DashboardPage() {
  ...
  // Citas agendadas hoy
  const todaysAppointmentsQuery = await db
  ...
  const egresosDia = todaysExpenses
  ...
  const pendingAccountsByClient = new Map<
    string,
    {
      clientId: string;
      clientName: string;
  ...
  const pendingAccounts = [...pendingAccountsByClient.values()].sort(
    (first, second) => second.remaining - first.remaining,
  );
  ...
  const inventoryAlerts = await db
    ...
    .from(products)
    .where(
      and(eq(products.organizationId, orgId), lte(products.currentStock, products.minimumStock)),
    )
    .limit(5);
  ...
  return (
    <div className="p-4 md:p-[30px] flex flex-col gap-4 md:gap-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 md:gap-6">
          ...
          <div className="bg-[#141414] border border-white/10 rounded-xl p-4 md:p-6 overflow-x-auto">
            ...
            {upcomingAppointments.length === 0 ? (
              ...
              <table className="w-full border-collapse min-w-[500px]">
                <thead>
                  <tr>
                    <th className="text-left py-3 px-2.5 text-xs text-charcoal font-medium border-b border-white/10">
                      Hora
                    </th>
                    <th className="text-left py-3 px-2.5 text-xs text-charcoal font-medium border-b border-white/10">
                      Cliente
                ...
                <tbody>
                  {upcomingAppointments.map((apt) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-2.5 text-sm text-sterling">{apt.serviceName}</td>
            ...
            <div className="bg-[#141414] border border-white/10 rounded-xl p-4 md:p-6">
              ...
              <ul className="flex flex-col gap-3">
                {inventoryAlerts.map((product) => (
                  <li
                    key={product.id}
                    className="flex justify-between items-center bg-red-900/10 border border-red-500/20 p-3 rounded-lg"
                  >
                    <div>
                      <h4 className="text-sm text-sterling">{product.name}</h4>
                      <p className="text-xs text-red-400">Stock actual: {product.currentStock}</p>
                    </div>
                    <span className="text-xs text-charcoal bg-white/5 px-2 py-1 rounded">
        ...
        <div className="flex flex-col gap-4 md:gap-6">
          <div className="bg-[#141414] border border-white/10 rounded-xl p-4 md:p-6">
            <h3 className="font-serif text-lg text-sterling mb-5 text-orange-400">
              Cuentas por Cobrar
            </h3>
              ...
              <ul className="flex flex-col gap-3">
                {pendingAccounts.map((account) => (
                  <li
                    ...
                    className="flex justify-between items-center gap-3 bg-orange-500/10 border border-orange-500/20 p-3 rounded-lg"
                  >
                    <div>
                      <h4 className="text-sm font-bold text-sterling">
                        {account.clientName} {account.clientLastName || ""}
                      </h4>
                      <p className="text-xs text-orange-400 font-medium mt-0.5">
                        Debe: ${account.remaining.toFixed(2)}
                      </p>
