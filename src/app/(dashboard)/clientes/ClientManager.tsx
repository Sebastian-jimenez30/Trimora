...
export default function ClientManager({ initialClients }: { initialClients: ClientType[] }) {
  ...
  const filteredClients = initialClients.filter((c) => {
    ...
    return (
      fullName.includes(term) || c.phone?.includes(term) || c.email?.toLowerCase().includes(term)
    );
  });
    ...
    <div className="flex flex-col gap-6 flex-1 h-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-[#141414] border border-white/10 rounded-xl p-4 shrink-0">
        <div className="relative w-full sm:w-auto">
          <svg
            className="w-4 h-4 absolute left-3 top-3 text-charcoal"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
              ...
              filteredClients.map((client) => (
                ...
                >
                  <td className="py-4 px-6">
                    <div className="text-sm font-medium text-sterling">
                      {client.firstName} {client.lastName}
                    </div>
                    {client.notes && (
                      <div className="text-xs text-charcoal mt-1 line-clamp-1">{client.notes}</div>
                    )}
                  </td>
                  <td className="py-4 px-6">
      ...
      {isModalOpen && (
        <Dialog
          label={editingClient ? "Editar Cliente" : "Nuevo Cliente"}
          onClose={closeModal}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                placeholder="Ej: Es alérgico a la cera caliente..."
              ></textarea>
            </div>
