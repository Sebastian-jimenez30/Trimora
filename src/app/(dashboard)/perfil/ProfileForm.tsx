    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div className="col-span-1">
        <div className="bg-midnight/90 rounded-2xl shadow-2xl border border-white/10 p-6 flex flex-col items-center text-center">
          ...
          {isPendingAvatar && (
            <p className="text-xs text-blue-400 mt-4 animate-pulse">Subiendo foto...</p>
          )}
          {avatarMsg.text && (
            <p
              className={`text-xs mt-4 ${avatarMsg.type === "success" ? "text-green-400" : "text-red-400"}`}
            >
              {avatarMsg.text}
            </p>
          )}
        ...
        <div className="bg-midnight/90 rounded-2xl shadow-2xl border border-white/10 p-6 md:p-8">
          ...
          <form onSubmit={handleInfoSubmit} className="space-y-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="block text-sm font-medium text-sterling mb-1.5">
                  Nombre Completo
                </label>
                <input
                  type="text"
            ...
            <div className="pt-2 flex items-center justify-between">
              <div>
                {infoMsg.text && (
                  <span
                    className={`text-sm ${infoMsg.type === "success" ? "text-green-400" : "text-red-400"}`}
                  >
                    {infoMsg.text}
                  </span>
                )}
          ...
          <form onSubmit={handlePwdSubmit} className="space-y-5">
            ...
            <div className="pt-2 flex items-center justify-between">
              <div>
                {pwdMsg.text && (
                  <span
                    className={`text-sm ${pwdMsg.type === "success" ? "text-green-400" : "text-red-400"}`}
                  >
                    {pwdMsg.text}
                  </span>
                )}
