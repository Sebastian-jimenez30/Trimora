    ...
    <div className="flex flex-col gap-6 flex-1 h-full">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center bg-[#141414] p-4 rounded-xl border border-white/10 gap-4 shrink-0">
        <div className="relative flex items-center w-full sm:w-auto">
          <svg
            className="w-4 h-4 absolute left-3 text-charcoal"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
        ...
        <table className="w-full text-left border-collapse min-w-[700px]">
          <thead className="sticky top-0 bg-[#141414] shadow-sm z-10">
            <tr className="border-b border-white/10 bg-white/5">
              <th className="py-4 px-6 text-xs text-charcoal font-medium uppercase tracking-wider">
                Producto
              </th>
              <th className="py-4 px-6 text-xs text-charcoal font-medium uppercase tracking-wider">
                Categoría
              ...
              <th className="py-4 px-6 text-xs text-charcoal font-medium uppercase tracking-wider text-right">
                Ajuste Rápido
              </th>
            </tr>
          </thead>
                  ...
                  >
                    <td className="py-4 px-6">
                      <div className="flex items-start justify-between group">
                        <div>
                          <div
                            className="text-sm font-medium text-sterling group-hover:text-cognac transition-colors cursor-pointer"
                            onClick={() => openEditModal(product)}
                          >
                            {product.name}
                          </div>
                          {product.description && (
                            <div className="text-xs text-charcoal mt-1 line-clamp-1">
                              {product.description}
                            </div>
                          )}
                        </div>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-2">
                        <span
                          className={`text-sm font-bold ${isLowStock ? "text-red-400" : "text-sterling"}`}
                        >
                        ...
                        {isLowStock && (
                          <div
                            className="w-2 h-2 rounded-full bg-red-500 animate-pulse ml-1"
                            title="Stock bajo"
                          ></div>
                        )}
                      </div>
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end gap-3">
                        <div className="flex items-center bg-pitch border border-white/10 rounded-lg overflow-hidden">
                          ...
                          <span
                            className={`w-12 text-center text-sm font-medium ${delta > 0 ? "text-green-400" : delta < 0 ? "text-red-400" : "text-sterling"}`}
                          >
                            {delta > 0 ? `+${delta}` : delta}
                          </span>
                          <button
          ...
          <div className="bg-[#141414] border border-white/10 w-full max-w-md max-h-[90vh] overflow-y-auto rounded-xl shadow-2xl p-5 md:p-7 animate-in zoom-in-95 duration-200">
            ...
            {(() => {
                ...
                return (
                  <div className="flex flex-col gap-4">
                    <p className="text-sm text-charcoal">
                      El stock de{" "}
                      <strong className="text-sterling">{confirmingProduct.name}</strong> llegará a
                      0.
                    </p>
                    <p className="text-sm text-charcoal">
                      ¿Deseas mantenerlo en el catálogo con stock 0, o prefieres eliminar el
                      producto definitivamente del sistema?
                    </p>
              ...
              return (
                <div className="flex flex-col gap-4">
                  <p className="text-sm text-charcoal">
                    Estás a punto de actualizar el stock de{" "}
                    <strong className="text-sterling">{confirmingProduct.name}</strong>.
                  </p>
                  <div className="flex items-center justify-between bg-white/5 p-4 rounded-lg">
                    ...
                    <div className="text-center">
                      ...
                      <p
                        className={`text-lg font-bold ${delta > 0 ? "text-green-400" : "text-red-400"}`}
                      >
                        {delta > 0 ? `+${delta}` : delta}
                      </p>
                    </div>
                  ...
                  <div className="flex justify-end gap-3 mt-4">
                    <button
                      onClick={closeConfirmModal}
                      className="px-5 py-2 text-sm text-sterling hover:bg-white/5 rounded-lg transition-colors font-medium"
                    >
                      Cancelar
                    </button>
                    <button
      ...
      {isModalOpen && (
        <Dialog
          label={editingProduct ? "Editar Producto" : "Nuevo Producto"}
          ...
          <form onSubmit={handleCreateSubmit} className="flex flex-col gap-5">
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5 col-span-2">
                ...
                <input
                  ...
                  className="bg-pitch border border-white/10 text-sterling px-3 py-2.5 rounded-lg text-sm focus:outline-none focus:border-cognac"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <select
                  name="category"
                  required
                  defaultValue={editingProduct?.category || "VENTA"}
                  className="bg-pitch border border-white/10 text-sterling px-3 py-2.5 rounded-lg text-sm focus:outline-none focus:border-cognac"
                >
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                className="bg-pitch border border-white/10 text-sterling px-3 py-2.5 rounded-lg text-sm focus:outline-none focus:border-cognac resize-none"
              ></textarea>
            </div>
            ...
            <div className="flex justify-between gap-3 mt-2 border-t border-white/10 pt-5">
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="submit"
                  disabled={isPending}
                  className="bg-cognac hover:brightness-110 text-white px-6 py-2.5 rounded-lg text-sm font-medium transition-all disabled:opacity-50 flex items-center gap-2"
                >
                  {isPending && (
                    <svg
