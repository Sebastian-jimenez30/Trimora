...
export type AgendaService = { id: string; name: string; durationMinutes: number; price: string };
export type AgendaStaff = { id: string; name: string; role: string };
...
function getClientLabel(client: AgendaClient) {
...
export default function AgendaManager({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const baseDate = editingAppointment
      ? editingAppointment.startTime
      : selectedDateForNew || currentDate;
  ...
  const getEffectiveEndTime = (appointment: Appointment) => {
    ...
    return new Date(
      Date.parse(appointment.startTime) + service.durationMinutes * 60 * 1000,
    ).toISOString();
  };
  ...
  const getAppointmentStyle = (app: Appointment, layout?: AppointmentLayout) => {
    ...
    const [endHour, endMinute] = getBogotaTimeValue(getEffectiveEndTime(app))
      .split(":")
      .map(Number);
    ...
    if (app.status === "COMPLETED")
      bgClass = "bg-[#1a1a1a] border-l-[4px] border-[#888] opacity-70";
  ...
  const renderDayView = () => {
    ...
    const dayLayouts = getAppointmentLayouts(dayApps);
    const now = new Date();
            ...
            >
              {hours.map((h, i) => (
                <div
                  key={`slot-${h}`}
                  onClick={() =>
                    openCreateModal(`${h.toString().padStart(2, "0")}:00`, currentDate)
                  }
                  className="absolute w-full h-[60px] cursor-pointer hover:bg-white/[0.03] transition-colors"
                  style={{ top: `${i * 60}px` }}
                />
              ))}
              ...
              {isToday && now.getHours() >= 5 && now.getHours() <= 22 && (
                <div
                  className="absolute w-full border-t-[2px] border-cognac z-30 pointer-events-none"
                  style={{ top: `${(now.getHours() - 5) * 60 + now.getMinutes()}px` }}
                >
              ...
              {dayApps.map((app) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-[10px] text-[#ccc] truncate mt-0.5">
                      {service?.name} • {getBogotaTime(app.startTime)}
                    </div>
                  </div>
                );
              })}
          ...
          <div className="flex-1 grid grid-cols-7 min-w-[500px]">
            {weekDays.map((d) => {
              ...
              return (
                <div
                  key={d.toString()}
                  className="flex flex-col items-center border-r border-white/10 last:border-0 cursor-pointer hover:bg-white/5"
                  onClick={() => {
                    setCurrentDate(d);
                    setView("day");
                  }}
                >
                  <span
                    className={`text-[10px] uppercase font-medium tracking-wide ${isToday ? "text-cognac" : "text-[#888]"}`}
                  >
                    {d.toLocaleDateString("es-ES", { weekday: "short" })}
                  </span>
                  <div
                    className={`w-8 h-8 flex items-center justify-center rounded-full mt-1 text-sm ${isToday ? "bg-cognac text-white font-bold" : "text-sterling"}`}
                  >
                    {d.getDate()}
                  </div>
                </div>
                ...
                return (
                  ...
                  >
                    {hours.map((h, i) => (
                      <div
                        key={`slot-${h}`}
                        onClick={() => openCreateModal(`${h.toString().padStart(2, "0")}:00`, d)}
                        className="absolute w-full h-[60px] cursor-pointer hover:bg-white/[0.03] transition-colors"
                        style={{ top: `${i * 60}px` }}
                      />
                    ))}
                    ...
                    {apps.map((app) => {
                      ...
                      return (
                        <div
                          key={app.id}
                          className={`${style.className} !rounded-sm !border-l-[3px]`}
                          style={{
                            top: style.top,
                            left: style.left,
                            width: style.width,
                        ...
                        >
                          <div className="font-bold text-white text-[10px] truncate leading-tight">
                            {client?.firstName}
                          </div>
                        </div>
                      );
      ...
      <div className="flex-1 flex flex-col bg-[#141414]">
        <div className="grid grid-cols-7 border-b border-white/10 shrink-0 bg-[#1a1a1a]">
          {weekDayNames.map((d) => (
            <div
              key={d}
              className="py-2 text-center text-[10px] font-medium text-[#888] uppercase tracking-wider"
            >
              {d}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7 grid-rows-5 flex-1 bg-[#141414]">
          {days.map((d) => {
            ...
            return (
              ...
              >
                <div
                  className={`text-center text-xs mb-1 mx-auto flex items-center justify-center w-6 h-6 rounded-full ${isToday ? "bg-cognac text-white font-bold" : isCurrentMonth ? "text-sterling" : "text-charcoal"}`}
                >
                  {d.getDate()}
                </div>
                <div className="flex-1 overflow-y-auto space-y-1 mt-1 px-1">
                  {apps.slice(0, 3).map((app) => {
                    ...
                    return (
                      <div
                        key={app.id}
                        className="text-[9px] bg-cognac text-white rounded-[4px] px-1.5 py-0.5 truncate shadow-sm"
                      >
                  ...
                  {apps.length > 3 && (
                    <div className="text-[10px] text-[#888] text-center font-medium">
                      +{apps.length - 3}
                    </div>
                  )}
                </div>
    ...
    >
      ...
      <header className="h-[64px] border-b border-white/10 flex items-center justify-between px-4 lg:px-6 bg-[#1a1a1a] shrink-0 shadow-sm z-30">
        <div className="flex items-center gap-2 md:gap-6">
          <div className="flex items-center gap-1 group relative">
            <h2 className="text-xl md:text-2xl font-serif text-white capitalize flex items-baseline gap-2">
              ...
              <span className="text-sm font-sans text-[#888] font-normal hidden md:inline">
        ...
        <div className="flex items-center gap-3 md:gap-5">
          ...
          <button
            onClick={setToday}
            className="md:hidden text-sterling p-2 hover:bg-white/5 rounded-full"
          >
            <svg
              width="20"
              height="20"
          ...
          <div className="flex bg-[#0f0f0f] rounded-lg p-1 border border-white/5 shadow-inner">
            <button
              onClick={() => setView("day")}
              className={`px-2 md:px-3 py-1.5 text-xs rounded-md font-medium transition-all ${view === "day" ? "bg-white/10 text-white shadow-sm" : "text-[#888] hover:text-white"}`}
            >
              Día
            </button>
            <button
              onClick={() => setView("week")}
              className={`px-2 md:px-3 py-1.5 text-xs rounded-md font-medium transition-all ${view === "week" ? "bg-white/10 text-white shadow-sm" : "text-[#888] hover:text-white"}`}
            >
              Semana
            </button>
            <button
              onClick={() => setView("month")}
              className={`px-2 md:px-3 py-1.5 text-xs rounded-md font-medium transition-all ${view === "month" ? "bg-white/10 text-white shadow-sm" : "text-[#888] hover:text-white"}`}
            >
              Mes
            </button>
          </div>
      ...
      {isModalOpen && (
        <Dialog
          label={editingAppointment ? "Detalles de la Cita" : "Nueva Cita"}
          onClose={closeModal}
          overlayClassName="z-[60] bg-black/70 animate-in fade-in duration-200"
          className="bg-[#141414] border border-white/10 w-full max-w-lg max-h-[90vh] overflow-y-auto rounded-xl shadow-2xl p-5 md:p-7 animate-in zoom-in-95 duration-200"
        >
          <div className="flex justify-between items-center mb-6 pb-4 border-b border-white/10">
            ...
            <button
              ...
              className="text-charcoal hover:text-white transition-colors bg-white/5 w-8 h-8 rounded-full flex items-center justify-center"
            >
              <svg
                width="18"
                height="18"
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                <div className="relative">
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      if (
                        clients.find((client) => getClientLabel(client) === value)?.id !==
                        selectedClientId
                      ) {
                        setSelectedClientId("");
                      }
                  ...
                  {isClientPickerOpen && (
                    <div
                      id="appointment-client-options"
                      role="listbox"
                      className="absolute z-20 mt-1 max-h-48 w-full overflow-y-auto rounded-lg border border-white/10 bg-[#1a1a1a] shadow-xl"
                    >
                      {clients
                        .filter((client) =>
                      ...
                      {clients.filter((client) =>
                        ...
                        <p className="px-3 py-2.5 text-sm text-charcoal">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  defaultValue={
                    editingAppointment
                      ? getBogotaTimeValue(editingAppointment.startTime)
                      : defaultTime
                  }
                  className="bg-pitch border border-white/10 text-sterling px-3 py-2.5 rounded-lg text-sm focus:outline-none focus:border-[#8B4513]"
                />
                <p className="text-[11px] text-charcoal">
                  Abre el selector para elegir una hora y minuto exactos.
                </p>
              </div>
              <div className="flex flex-col gap-1.5">
                <label className="text-xs text-charcoal uppercase tracking-wider">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <textarea
                ...
                placeholder="Ej. El cliente prefiere corte a tijera..."
                className="bg-pitch border border-white/10 text-sterling px-3 py-2.5 rounded-lg text-sm focus:outline-none focus:border-[#8B4513] resize-none"
              ></textarea>
            </div>
            ...
            <div className="flex justify-between mt-2 border-t border-white/10 pt-5">
              ...
              <div className="flex gap-3">
                <button
                  type="button"
                  onClick={closeModal}
                  className="px-5 py-2.5 text-sm text-sterling hover:bg-white/5 rounded-lg transition-colors font-medium"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={isPending}
                  className="bg-[#8B4513] hover:brightness-110 text-white px-6 py-2.5 rounded-lg text-sm font-medium transition-all disabled:opacity-50 flex items-center gap-2"
                >
            ...
            <form onSubmit={handleCreateClientSubmit} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  name="firstName"
                  required
                  className="bg-pitch border border-white/10 text-sterling px-3 py-2 rounded-lg text-sm focus:outline-none focus:border-[#8B4513]"
                />
              </div>
              <div className="flex flex-col gap-1.5">
              ...
              <div className="flex justify-end gap-3 mt-4">
                <button
                  type="button"
                  onClick={() => setIsClientModalOpen(false)}
                  className="px-4 py-2 text-sm text-sterling hover:bg-white/5 rounded-lg transition-colors"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={isPending}
                  className="bg-[#8B4513] hover:brightness-110 text-white px-5 py-2 rounded-lg text-sm font-medium transition-all disabled:opacity-50"
                >
