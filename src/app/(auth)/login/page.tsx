...
export default async function LoginPage(props: {
  searchParams: Promise<{ message?: string; invite_token?: string }>;
}) {
      ...
      <div className="max-w-md w-full bg-midnight/90 rounded-2xl shadow-2xl overflow-hidden border border-white/10 backdrop-blur-xl">
        <div className="p-10">
          ...
          <form action={login} className="space-y-6">
            ...
            <div>
              <label htmlFor="login-email" className="block text-sm font-medium text-sterling mb-2">
                Correo Electrónico
              </label>
              <input
                id="login-email"
            ...
            <div>
              <div className="flex justify-between items-center mb-2">
                <label htmlFor="login-password" className="block text-sm font-medium text-sterling">
                  Contraseña
                </label>
                <Link
                  href="/forgot-password"
                  className="text-xs text-cognac hover:text-cognac-hover font-bold transition-colors"
                >
            ...
            <div className="pt-4 flex flex-col gap-4">
              ...
              <div className="mt-8 text-center border-t border-gray-800 pt-6">
                <p className="text-sterling/70 text-sm">Solo puedes unirte si fuiste invitado.</p>
              </div>
            </div>
          </form>
