...
- **`supabaseAdmin` (Service Role):** Se provee una instancia de cliente Supabase con la llave maestra _solo para uso del lado del servidor_ (acciones críticas como crear usuarios). Nunca se expone al cliente.
...
- **`FeaturesCarousel` (`src/components/FeaturesCarousel.tsx`):** Un componente animado que se renderiza en la _Landing Page_. Expone visualmente el valor del ecosistema (Agenda, Clientela, Cobros, IA) usando tarjetas que transicionan en carrusel. No interactúa con la DB, es puramente presentacional.
